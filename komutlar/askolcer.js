...
exports.run = async (client, message, args) => {
    ...
    var yorum = `Bence evlenmelisiniz..`
    if(anasonuc < 80) {
        var yorum = 'Deliler gibi aşık!'
    }
    if(anasonuc < 60) {
        var yorum = 'Eh işte, arada trip atıyor.'
    }
    if(anasonuc < 40) {
        var yorum = 'Sana karşı bir şeyler hisettiğine yemin edebilirim.'
    }
    if(anasonuc < 20) {
        var yorum = 'Maalesef, bu aşk yürümez.'
    }
    const embed = new Discord.MessageEmbed()
        .setAuthor(`${member.user.tag} Ve ${s.tag} aranızdaki aşk sonucu.`)
