...
if(!muteRoleFetch) return message.channel.send('Bu sunucuda ``MUTE`` rolü ayarlanmamış, `+muterole` yazarak ayarla veya rolün yoksa `+muterole olustur [rolisim]` komutunu kullanabilirsin!');
...
return message.channel.send(`**${message.author.tag}** adlı yetkili **${member.user.tag}** adlı kişiyi sonsuza kadar susturdu. ${reason}`);
});
} else {
...
if(ms(zaman) >= 2147483647) return message.channel.send('24 saat içinde 1 kez mute atabilirsin.');// ellemeyin arkadaslar.
...
exports.conf = {
  enabled: true,
  guildOnly: true,
  aliases: ['sustur','mutele','muteat','mute-at'],
  permLevel: 0
};
